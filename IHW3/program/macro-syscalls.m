...
#-------------------------------------------------------------------------------
# Вызов подпрограммы чтения файла и его закрытия в случае аварийного завершения
# или по окончании работы с файлом
.macro read_close_file(%file_descriptor, %reg_str, %reg_len)
	mv a0, %file_descriptor
	jal read_close_file
	mv %reg_str, a0 # address of text start
	mv %reg_len, a1 # len of text
.end_macro

#-------------------------------------------------------------------------------
# Вызов основной подпрограммы-алгоритма
.macro capitalize_vowels(%x)
	mv a0, %x
	jal capitalize_vowels
.end_macro
...
#-------------------------------------------------------------------------------
# Ввод символа с консоли в указанный регистр, исключая регистр a0
.macro read_char(%x)
   push	(a0)
   li a7, 12
   ecall
...
#-------------------------------------------------------------------------------
# Открытие файла с проверкой и выбросом из программы при ошибке
.macro open_fatal_reg(%file_name, %opt, %reg)
    open(%file_name, %opt)
    mv %reg, a0
	li t0, -1
	bne %reg, t0, open_fatal_reg_continue
	fatal
	
open_fatal_reg_continue:
...
.macro write_addr(%file_descriptor, %reg, %size_reg)
    li   a7, 64       	# Системный вызов для чтения из файла
    mv   a0, %file_descriptor       # Дескриптор файла
    mv   a1, %reg   	# Адрес буфера для читаемого текста из регистра
    mv   a2, %size_reg 		# Размер читаемой порции
    ecall             	# Чтение
.end_macro
