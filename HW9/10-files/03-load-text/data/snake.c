// Для компиляции программы введите clang zmeyka.c -o c-zmeyka
...
void change_direction() {
  ...
  switch (symbol) {
    ...
    case 'k': if (change_x != 1 || change_y != 0) { // up
                change_x = -1; change_y = 0;
              }
              break;
    //case 'a': if (change_x != 0 || change_y != 1) {
    case 'h': if (change_x != 0 || change_y != 1) { // left
                change_x = 0; change_y = -1;
              }
              break;
    //case 's': if (change_x != -1 || change_y != 0) {
    case 'j': if (change_x != -1 || change_y != 0) { // down
                change_x = 1; change_y = 0;
              }
              break;
    //case 'd': if (change_x != 0 || change_y != -1) {
    case 'l': if (change_x != 0 || change_y != -1) {
                change_x = 0; change_y = 1;
              }
              break;
#ifndef WINDOWS
